...
section("06 · H3: hexagons, and when they beat PostGIS (solution)");
info(state.detail);
...
//
// latLngToCell takes LATITUDE first. ST_MakePoint takes LONGITUDE first. They are
// opposite, and swapping them does not error; it puts your point somewhere real
// and wrong. Worth a comment at every boundary between the two.
...
check("the cell is valid", isValidCell(cell));
...
const h3Run = await time(
  async () => {
    ...
    for (const r of rows) {
      const c = latLngToCell(Number(r.lat), Number(r.lon), 7);
...
check("every plot landed in a cell", [...buckets.values()].reduce((n, b) => n + b.plots, 0) === rows.length);
...
const assign = await time(
  async () => {
    ...
    for (const r of all) {
      ...
      cells.push(latLngToCell(Number(r.lat), Number(r.lon), 7));
    }
    await q(
...
const covering = polygonToCells(boxAroundNashik, 8);
measure("cells covering a ~20 km box at resolution 8", covering.length.toLocaleString());
check("the box is covered by a plausible number of cells", covering.length > 50 && covering.length < 5_000);
