...
section("05 · PostGIS: geometry, geography and the spatial index");
info(state.detail);
...
// TODO 1 — create the spatial index, then ANALYZE.
//   CREATE INDEX plots_geog_gist ON plots USING GIST ((geom::geography))
//
// Note the double brackets. The column is geometry and the query casts to geography,
// so the index has to be on that expression. An index on `geom` alone will not be
// used here, and it looks exactly like a broken index rather than a wrong one.
...
check("the radius query now uses the spatial index", indexedPlan.indexes.includes("plots_geog_gist"), "TODO 1");
check(
...
//
// The nearest-neighbour query orders by `geom <-> point`. Look at what that expression
// is, and at what the index from step 3 is on, and predict whether it can help.
...
// TODO 2 — create a second index, this time on the plain geometry.
//   CREATE INDEX plots_geom_gist ON plots USING GIST (geom)
//
// Before you do: look at what the query above measured without it. Two indexes on
// one column sounds wasteful. Decide whether it is, then measure.
...
section("Checks");
check("both spatial indexes exist", (await q(`SELECT 1 FROM pg_indexes WHERE indexname IN ('plots_geog_gist', 'plots_geom_gist')`)).length === 2, "TODO 1 and TODO 2");
finish();
await close();
